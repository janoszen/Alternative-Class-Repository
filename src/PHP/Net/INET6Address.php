...
/**
 * This class represents IPv6 addresses
 */
class INET6Address extends INETAddress {
	 ...
	 * @throws INETAddressFormatError if the address is not a valid IPv6 address
	 * @return INET6Address
	 */
	...
	/**
	 * Returns the address as a binary representation, optinally masking the given number of bits.
	...
	public function getAsBinary($mask = null) {
		...
		if (is_int($mask) && $mask > -1 && $mask < 129) {
			...
			throw new INETInvalidMask($mask);
		}
		return $binary;
	}
	...
	public function isGloballyRoutable() {
		...
		if ($this->isInRanges(self::$globallyroutableexc)) {
			return false;
		} else if ($this->isInRanges(self::$globallyroutableaddr)) {
			return true;
		} else {
			return false;
		}
