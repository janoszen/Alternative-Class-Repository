...
class File {
	...
	public function createNewFile() {
		if (!file_exists($this->pathname)) {
			if (!$this->canWrite()) {
				throw new \PHP\IO\IOException("Access denied, file creation of " . $this->pathname . " failed");
			} else if (!touch($this->pathname)) {
	...
	public function isAbsolute() {
		if ((substr($this->pathname, 0, 1) != self::getPathSeparator())
				&& (substr($this->pathname, 1, 1 + strlen(self::getPathSeparator())) != ":" . self::getPathSeparator())) {
			return false;
		} else {
			return true;
		}
